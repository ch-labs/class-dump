...
- (BOOL)hasObjectiveC2Data;
{
    ...
    // Mach-O files inside archives (static libraries) don't have a __DATA segment, but one unnamed segment with all the sections
    for (CDLCSegment *segment in self.segments) {
        for (CDSection *section in segment.sections) {
            if (([section.segmentName isEqualToString:@"__DATA"] && [section.sectionName isEqualToString:@"__objc_imageinfo"]) ||
                ([section.segmentName isEqualToString:@"__OBJC"] && [section.sectionName isEqualToString:@"__image_info"]))
                return YES;
        }
    }
